...
export async function runMvpSprint(missionId, apiPreference = "auto", onEvent = async () => {}) {
  const capabilityInstruction = apiPreference === "auto"
    ? "API mode is Auto. Theo may select one or two complementary supported Google capabilities."
    : `The founder selected ${apiPreference}. Theo must use exactly that one Google capability and must not add or substitute another capability.`;
  const contents = [{
    role: "user",
    parts: [{

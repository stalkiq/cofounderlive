...
import { runCofounderAsk } from "./workspace.mjs";
...
export async function getImplementationState(missionId) {
  ...
  return {
    ...
    cofounderReply: data.cofounderReply || "",
...
export async function runAntigravityImplementation(missionId, instruction, onEvent = async () => {}, options = {}) {
  ...
  try {
    const ack = await runCofounderAsk(missionId, request, {
      cofounder: "theo",
      idea: record.idea || "",
      voice: revise ? "build_ack" : "ask",
    });
    if (ack?.answer) {
      await onEvent({
        type: "cofounder_reply",
        agent: "technical",
        text: ack.answer,
        proof: { worker: "theo", voice: revise ? "build_ack" : "ask", stage: "briefing" },
      });
    }
  } catch {
    await onEvent({
      ...
      agent: "technical",
      text: revise
        ? `On it — I’ll apply “${request.slice(0, 80)}” to the live app without wiping what we already built.`
        : "On it — I’m spinning up the coding agents for the real app now.",
      proof: { worker: "theo", voice: "fallback", stage: "briefing" },
    });
  }
  ...
  try {
    ...
    if (!result.ok || !result.files?.length) {
      await onEvent({
        ...
        agent: "technical",
        text: revise
          ? "Theo fell back to Gemini to apply your change on the existing app files."
          : "Theo switched to a Gemini scaffold while keeping the Antigravity delivery shape.",
        proof: {
          worker: "theo",
          status: "recovering",
          task: revise
            ? "Apply founder change with Gemini revise fallback."
            : "Generate a deployable multi-screen scaffold for follow-up Antigravity work.",
          stage: "scaffold-api",
        },
      });
    ...
    try {
      const doneReply = await runCofounderAsk(missionId, request, {
        cofounder: "theo",
        idea: record.idea || "",
        voice: "build_done",
        outcome: revise ? "change applied and verified" : "real app delivered and verified",
      });
      if (doneReply?.answer) {
        proof.cofounderReply = doneReply.answer;
        await onEvent({
          type: "cofounder_reply",
          agent: "technical",
          text: doneReply.answer,
          proof: { worker: "theo", voice: "build_done", stage: "complete", previewAppUrl: proof.previewAppUrl },
        });
      }
    } catch {
      const fallbackReply = revise
        ? `Done — I applied “${request.slice(0, 80)}”. Open App preview to check it.`
        : "Done — the real app is ready. Open App preview to try it.";
      proof.cofounderReply = fallbackReply;
      await onEvent({
        type: "cofounder_reply",
        agent: "technical",
        text: fallbackReply,
        proof: { worker: "theo", voice: "fallback", stage: "complete", previewAppUrl: proof.previewAppUrl },
      });
    }

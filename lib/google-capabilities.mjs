...
export function normalizeGoogleCapabilityPreference(value) {
  const preference = String(value || "auto").trim().toLowerCase();
  return GOOGLE_CAPABILITIES.includes(preference) ? preference : "auto";
}
...
export function applyGoogleCapabilityPreference(mvp, preference, previousMvp = {}) {
  const normalizedPreference = normalizeGoogleCapabilityPreference(preference);
  const current = normalizeGoogleCapabilities(mvp?.googleCapabilities || mvp?.googleCapability);
  if (normalizedPreference === "auto") {
    mvp.googleCapabilities = current;
    mvp.googleCapability = current[0];
    return current;
  }

  const previous = normalizeGoogleCapabilities(
    previousMvp?.googleCapabilities || previousMvp?.googleCapability || [],
  );
  const selected = current.find((capability) => capability.service === normalizedPreference)
    || previous.find((capability) => capability.service === normalizedPreference)
    || normalizeGoogleCapability({ service: normalizedPreference });
  const capabilities = [normalizeGoogleCapability(selected)];

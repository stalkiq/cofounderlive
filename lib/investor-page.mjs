...
import { capabilitiesFor } from "./google-capabilities.mjs";
...
const MAPS_BROWSER_KEY = process.env.MAPS_BROWSER_KEY || "";
...
function screenBody(screen, mvp, index) {
  ...
  const capabilities = capabilitiesFor(mvp);
  ...
  if (screen.type === "table") {
    const bigQueryCapability = capabilities.find((capability) => capability.service === "bigquery");
    const liveBigQuery = bigQueryCapability && Number(bigQueryCapability.primaryScreen) === index;
  ...
  if (screen.type === "map") {
    return `${intro}<div class="map-layout"><div class="map-canvas" data-google-map><div class="map-loading">Loading Google Maps…</div></div>
      <div class="map-list"><div class="capability-readout" data-geo-readout>${itemRows(items)}</div></div></div>`;
  }
  if (screen.type === "assistant") {
    const visionInput = services.includes("vision")
      ? `<label class="vision-upload">Image for Cloud Vision<input type="file" name="image" accept="image/*" /></label>`
      : "";
    const targetLanguage = capabilities.find((capability) => capability.service === "translation")?.config?.targetLanguage || "es";
    return `${intro}<div class="assistant-panel panel"><div class="assistant-mark">✦</div>
      <form class="local-composer assistant-form" data-composer="assistant">${visionInput}<textarea name="message" placeholder="${esc(screen.description || "What would you like to explore?")}"></textarea>
      ${services.includes("translation") ? `<input name="targetLanguage" value="${esc(targetLanguage)}" aria-label="Translation language code" />` : ""}
      <button class="primary">${services.includes("vision") ? "Analyze" : services.includes("translation") ? "Translate" : "Generate"}</button></form>
...
export function renderMvpPage(record) {
  ...
  const capabilities = capabilitiesFor(mvp);
  const capabilityLabel = capabilities.map((capability) => capability.label).join(" + ");
  ...
  const mapsBrowserKey = Object.prototype.hasOwnProperty.call(record, "mapsBrowserKey")
    ? String(record.mapsBrowserKey || "")
    : MAPS_BROWSER_KEY;
  const safeModel = JSON.stringify({
    missionId: record.missionId || "",
    apiBase: record.runtimeBaseUrl || "",
    capability: mvp.googleCapability || null,
    capabilities: capabilitiesFor(mvp),
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;min-height:100vh;background:var(--bg);color:var(--text);font-family:"DM Sans",sans-serif}button,input,select,textarea{font:inherit}
    ...
    .map-layout{display:grid;grid-template-columns:1.6fr 1fr;gap:var(--gap)}.map-canvas{position:relative;min-height:420px;overflow:hidden;background:color-mix(in srgb,var(--surface) 82%,var(--bg));border:1px solid var(--line);border-radius:var(--radius)}.map-loading{position:absolute;inset:0;display:grid;place-items:center;color:var(--muted);font:11px "IBM Plex Mono"}.map-list{padding:16px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);overflow:auto;max-height:420px}.capability-readout{display:grid;gap:12px}.readout-card{padding:14px;border:1px solid var(--line);border-radius:var(--radius);background:color-mix(in srgb,var(--primary) 5%,var(--surface))}.readout-card h3,.readout-card p{margin:0}.readout-card h3{font-size:13px}.readout-card p{margin-top:6px;color:var(--muted);font-size:11px;line-height:1.45}.readout-card a{color:var(--primary)}
    .assistant-panel{max-width:820px;text-align:center;padding:40px}.assistant-mark{font-size:46px;color:var(--accent)}.assistant-form{display:block}.assistant-form textarea{min-height:130px;resize:vertical;margin-bottom:10px}.assistant-form>input{margin-bottom:10px}.vision-upload{display:block;text-align:left;margin-bottom:12px}.vision-upload input{padding:9px}.assistant-output{display:none;text-align:left;white-space:pre-wrap;margin-top:20px;padding:20px;background:color-mix(in srgb,var(--primary) 9%,var(--bg));border-radius:var(--radius);line-height:1.6}.assistant-output.show{display:block}
    ...
    body.device-mobile{overflow:hidden}.nav-bottom .shell{display:block}.nav-bottom main{width:100%;min-height:100vh;padding-bottom:110px}.device-mobile .app-frame{max-width:430px;height:calc(100vh - 56px);min-height:0;margin:28px auto;border:1px solid var(--line);border-radius:32px;overflow:hidden;box-shadow:0 30px 90px color-mix(in srgb,var(--text) 10%,transparent)}.device-mobile .shell{grid-template-columns:1fr;height:100%;min-height:0}.device-mobile main{height:100%;padding:26px 20px 100px;overflow-y:auto;overscroll-behavior:contain}.device-mobile aside,.nav-bottom aside{position:fixed;z-index:10;left:50%;bottom:18px;transform:translateX(-50%);width:min(390px,calc(100% - 28px));display:flex;align-items:center;gap:4px;padding:8px;background:color-mix(in srgb,var(--surface) 94%,transparent);border:1px solid var(--line);border-radius:22px;backdrop-filter:blur(18px)}.device-mobile .logo,.device-mobile .built-by,.nav-bottom .logo,.nav-bottom .built-by{display:none}.device-mobile .nav-button,.nav-bottom .nav-button{flex:1;width:auto;margin:0;padding:11px 5px;text-align:center;font-size:10px}.device-mobile .metrics{grid-template-columns:1fr 1fr}.device-mobile .catalog-grid{grid-template-columns:1fr}.device-mobile .map-layout{grid-template-columns:1fr}.device-mobile .map-canvas{min-height:280px}.device-mobile .kanban{grid-template-columns:repeat(3,78%)}.device-mobile .screen-head h2{font-size:25px}
    @media(max-width:800px){.shell{grid-template-columns:1fr}body:not(.nav-topbar):not(.nav-bottom):not(.device-mobile) aside{display:none}main{padding:24px 18px 110px}.metrics{grid-template-columns:1fr 1fr}.catalog-grid{grid-template-columns:1fr}.map-layout{grid-template-columns:1fr}.kanban{grid-template-columns:repeat(3,80%)}.top{margin-bottom:30px}}
  </style>
</head>
<body class="archetype-${archetype} device-${device} nav-${navStyle} density-${density} type-${typeStyle} radius-${radius}">
  <div class="app-frame"><div class="shell">
    <aside><div class="logo">${esc(name)}<span class="prototype">MVP</span></div>
      ...
      <p class="built-by">Creative-reviewed · Technical-built<br/>Powered by ${esc(capabilityLabel || "Gemini on Google Cloud")}</p>
    </aside>
    <main>
      <header class="top"><p>${esc(mvp.workspaceLabel || "Founder workspace")}</p><span class="status">${esc(capabilityLabel || "Google Cloud")} · revision ${esc(record.revision || 1)}</span></header>
  ...
  <script>
    ...
    const services=(model.capabilities||[]).map(capability=>capability.service);
    const has=service=>services.includes(service);
    const api=path=>(model.apiBase||"")+path;
    async function jsonFetch(path,options){const response=await fetch(api(path),options),payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||"Google capability request failed");return payload}
    function readoutCard(title,detail,href){const article=document.createElement("article");article.className="readout-card";const h=document.createElement("h3"),p=document.createElement("p");h.textContent=title;p.textContent=detail;article.append(h,p);if(href){const a=document.createElement("a");a.href=href;a.target="_blank";a.rel="noreferrer";a.textContent="Open in Google Maps ↗";article.append(a)}return article}
    function loadMaps(){return new Promise((resolve,reject)=>{if(window.google?.maps)return resolve();if(!model.mapsKey)return reject(new Error("Google Maps browser key is not configured."));const script=document.createElement("script");script.src="https://maps.googleapis.com/maps/api/js?key="+encodeURIComponent(model.mapsKey)+"&libraries=geometry&v=weekly";script.async=true;script.onload=resolve;script.onerror=()=>reject(new Error("Google Maps could not load."));document.head.append(script)})}
    async function initGeo(){const canvas=document.querySelector("[data-google-map]"),readout=document.querySelector("[data-geo-readout]");if(!canvas)return;try{const [data]=await Promise.all([jsonFetch("/api/mvp/"+encodeURIComponent(model.missionId)+"/capabilities/geo"),loadMaps()]);canvas.replaceChildren();const isGlobal=/global|world/i.test(data.center.address||"");const map=new google.maps.Map(canvas,{center:{lat:data.center.lat,lng:data.center.lng},zoom:isGlobal?2:data.places?.length?11:7,mapTypeControl:true,streetViewControl:false,fullscreenControl:true});readout.replaceChildren();if(data.places?.length){data.places.forEach(place=>{if(Number.isFinite(place.lat)&&Number.isFinite(place.lng))new google.maps.Marker({map,position:{lat:place.lat,lng:place.lng},title:place.name});readout.append(readoutCard(place.name,[place.address,place.rating?"Rating "+place.rating:""].filter(Boolean).join(" · "),place.mapsUrl))})}else{new google.maps.Marker({map,position:{lat:data.center.lat,lng:data.center.lng},title:data.center.address});readout.append(readoutCard("Mapped location",data.center.address))}if(data.route?.polyline&&google.maps.geometry){const path=google.maps.geometry.encoding.decodePath(data.route.polyline);new google.maps.Polyline({map,path,strokeColor:getComputedStyle(document.body).getPropertyValue("--primary").trim()||"#4285f4",strokeOpacity:.9,strokeWeight:5});const bounds=new google.maps.LatLngBounds();path.forEach(point=>bounds.extend(point));map.fitBounds(bounds);readout.prepend(readoutCard("Live route",data.route.origin+" → "+data.route.destination+" · "+data.route.distance+" · "+data.route.duration))}if(data.weather)readout.prepend(readoutCard(data.weather.description+" · "+data.weather.location,[data.weather.temperature!=null?data.weather.temperature+"°F":"",data.weather.humidity!=null?"Humidity "+data.weather.humidity+"%":"",data.weather.wind!=null?"Wind "+data.weather.wind+" mph":""].filter(Boolean).join(" · ")));if(data.airQuality)readout.prepend(readoutCard("Air quality · "+data.airQuality.location+" · "+(data.airQuality.category||"Current"),["AQI "+(data.airQuality.aqi??"—"),data.airQuality.dominantPollutant?"Dominant "+data.airQuality.dominantPollutant:"",data.airQuality.recommendation].filter(Boolean).join(" · ")));(data.errors||[]).forEach(error=>readout.append(readoutCard(error.service+" needs attention",error.message)))}catch(error){canvas.replaceChildren(readoutCard("Map unavailable",error.message))}}
    function fileData(file){return new Promise((resolve,reject)=>{if(!file)return reject(new Error("Choose an image first."));if(file.size>1500000)return reject(new Error("Choose an image smaller than 1.5 MB."));const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(new Error("The image could not be read."));reader.readAsDataURL(file)})}
    async function speakResult(text){const response=await fetch(api("/tts"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:text.slice(0,1200),agent:"technical"})});if(!response.ok)throw new Error("Text-to-Speech failed");const audio=new Audio(URL.createObjectURL(await response.blob()));await audio.play()}
    ...
    document.querySelectorAll('[data-composer="assistant"]').forEach(form=>form.addEventListener("submit",async event=>{event.preventDefault();const output=form.parentElement.querySelector(".assistant-output"),message=form.elements.message.value.trim(),button=form.querySelector("button"),parts=[];output.textContent="Google capabilities are working…";output.classList.add("show");button.disabled=true;try{let workingText=message;if(has("vision")){const image=await fileData(form.elements.image?.files?.[0]);const result=await jsonFetch("/api/mvp/"+encodeURIComponent(model.missionId)+"/capabilities/vision",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image})});const labels=(result.labels||[]).map(label=>label.description+" "+label.score+"%").join(", ");workingText=result.text||labels;parts.push(["Cloud Vision",labels,result.text].filter(Boolean).join("\\n"))}if(has("gemini")){if(!workingText)throw new Error("Enter a prompt or choose an image.");const result=await jsonFetch("/api/mvp/"+encodeURIComponent(model.missionId)+"/assistant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:workingText})});workingText=result.text;parts.push("Gemini\\n"+result.text)}if(has("translation")){if(!workingText)throw new Error("Enter text or choose an image to translate.");const result=await jsonFetch("/api/mvp/"+encodeURIComponent(model.missionId)+"/capabilities/translate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:workingText,targetLanguage:form.elements.targetLanguage?.value})});workingText=result.text;parts.push("Cloud Translation ("+result.targetLanguage+")\\n"+result.text)}if(has("tts")){if(!workingText)throw new Error("Enter content to read aloud.");await speakResult(workingText);parts.push("Cloud Text-to-Speech\\nPlaying the generated audio.")}if(!parts.length)parts.push(model.assistantReply);output.textContent=parts.join("\\n\\n")}catch(error){output.textContent=error.message}finally{button.disabled=false}}));

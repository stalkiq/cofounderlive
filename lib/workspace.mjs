...
import { getInvestorPage, getMvpPage, saveMvpPage } from "./investor-page.mjs";
...
export async function runCofounderAsk(missionId, instruction, options = {}) {
  const record = await getMvpPage(missionId) || await getInvestorPage(missionId);
  if (!record) {
    throw new Error("Build a product concept before asking your cofounders.");
  }
...
${JSON.stringify({
  ...
  apiPreference: record.apiPreference || "auto",
  revision: record.revision || null,
})}

Return ONLY JSON:
{
  "answer":"your reply as ${cofounder === "maya" ? "Maya" : "Theo"} in under 180 words"
}`);
  ...
  return {
    ...
    agent,
    mode: "ask",
    answer: text(answer?.answer, "I need a bit more detail to help with that."),
  };
}

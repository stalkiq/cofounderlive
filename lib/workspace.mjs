...
import { normalizeGoogleCapability, prepareGoogleCapability } from "./google-capabilities.mjs";
...
function sanitizeSpec(candidate, current) {
  ...
  return {
    ...
    googleCapability: normalizeGoogleCapability(next.googleCapability, previous.googleCapability),
    screens,
    workflow: {
...
Create a focused plan that respects the request and improves usability. Use only the supported controlled runtime: dashboard, catalog, timeline, kanban, chat, calendar, table, map, assistant, and workflow screens. Preserve or improve the product's single supported Google capability: Google Maps links, Google Calendar actions, BigQuery analytics, or Gemini assistance. Do not request other external models, credentials, arbitrary JavaScript, shell commands, package installation, or unsupported APIs.
...
- Preserve exactly one Google capability and keep it aligned with a compatible screen.
...
${MVP_SCHEMA}`);
  ...
  await prepareGoogleCapability(missionId, revisedMvp);
